...
export default function AboutSection() {
  ...
  return (
    <section id="about" className="snap-start snap-always h-screen">
      <PandaBackground className="h-full" count={10}>
        <div className="h-full px-[4vw] pt-[100px] pb-[2vh] flex flex-col gap-[2vh]">
          ...
          {/* Row 2: 3 Value Cards */}
          <div ref={valueCardsRef} className="grid grid-cols-3 gap-[1.5vw] shrink-0">
            {valueCards.map((card, i) => {
              ...
              return (
                <div
                  ...
                  className="animate-on-scroll fade-up rounded-[14px] border-2 border-white bg-white/95 px-[1.5vw] py-[2.5vh] shadow-[0_2px_0_0_#B9DFA0,0_4px_8px_oklch(0_0_0/0.05)] flex flex-col items-center text-center"
                >
                  <span className={`flex size-[clamp(36px,4.5vh,44px)] items-center justify-center rounded-full ring-2 ${card.bg} ${card.ring}`}>
                    <Icon className={`size-[clamp(18px,2.2vh,22px)] ${card.color}`} strokeWidth={2} />
                  </span>
          ...
          <div
            ref={storyRef}
            className="animate-on-scroll fade-up rounded-2xl border-2 border-white/60 bg-white/70 px-[3vw] py-[2vh] shadow-[0_2px_0_0_#B9DFA0,0_4px_8px_oklch(0_0 0/0.05)] flex-1 min-h-0 flex flex-col"
          >
