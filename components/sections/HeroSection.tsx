        ...
        <div className="relative w-full px-[3vw] pt-[calc(72px+2vh)] pb-[2vh] flex flex-col flex-1">
          <div className="flex-1 flex items-center">
            <div className="grid lg:grid-cols-2 gap-[3vw] items-center w-full">
              ...
              >
                <span className="inline-flex items-center gap-2 self-start rounded-full border-2 border-clay-green-700/20 bg-white/80 px-[1vw] py-[0.6vh] font-body text-[clamp(12px,1.3vw,15px)] font-medium text-clay-green-700 backdrop-blur-sm">
                  <BadgeIcon className="size-[clamp(14px,1.2vw,16px)] text-clay-green-700" />
                  {heroBadge.text}
                </span>
                ...
                <div className="flex items-start gap-3 rounded-2xl border-[3px] border-clay-cream-border bg-white p-[1.2vh]" style={{ boxShadow: clayShadow("cream") }}>
                  <span className="flex size-[clamp(36px,4vh,48px)] shrink-0 items-center justify-center rounded-full bg-clay-orange-500 text-white">
                    <InfoIcon className="size-[clamp(16px,1.8vh,20px)]" />
                  </span>
                  <div>
                ...
                <div className="flex flex-col sm:flex-row gap-[1vh] mt-[0.5vh] w-full max-w-md">
                  <Link
                    ...
                    className="flex-1 inline-flex h-[clamp(44px,6vh,56px)] items-center justify-center gap-2 rounded-[20px] bg-clay-orange-500 border-[3px] border-clay-orange-700 px-[2vw] font-display text-[clamp(14px,2vh,18px)] font-bold text-white transition-all duration-150 hover:translate-y-0.5 whitespace-nowrap"
                  ...
                  <Link
                    href={WHATSAPP_URL}
                    className="flex-1 inline-flex h-[clamp(44px,6vh,56px)] items-center justify-center gap-2 rounded-[20px] border-[3px] border-clay-green-700 bg-white px-[2vw] font-display text-[clamp(14px,2vh,18px)] font-bold text-clay-green-700 transition-all duration-150 hover:translate-y-0.5 whitespace-nowrap"
          ...
          {/* bottom bar: 4 chips */}
          <div ref={bottomRef} className="animate-on-scroll fade-up pt-[2vh]">
            <div className="flex flex-row items-end gap-[clamp(6px,0.8vw,12px)] w-full">
              {heroFeatureChips.map((item) => {
                ...
                return (
                  <div
                    key={item.line1}
                    className="flex items-center gap-[clamp(8px,0.8vw,12px)] rounded-[14px] border-2 border-clay-cream-border bg-white px-[clamp(12px,1.2vw,18px)] py-[clamp(10px,1.5vh,14px)] flex-1 min-w-0"
                    style={{ boxShadow: clayShadow("cream") }}
                  >
                    <span className={cn("flex size-[clamp(36px,4.5vh,44px)] shrink-0 items-center justify-center rounded-full text-white", item.color)}>
                      <Icon className="size-[clamp(18px,2.2vh,22px)]" />
                    </span>
