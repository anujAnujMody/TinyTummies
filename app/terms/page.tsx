...
import { terms } from "@/data/tiny-tummies";
import { ClayCard, ClayButton, Type } from "@/components/claymorph";
import SmoothScroll from "@/components/islands/SmoothScroll";

export const metadata: Metadata = { title: "Terms & Conditions | Tiny Tummies", description: "Tiny Tummies terms, subscription and delivery conditions." };
        ...
        <div className="mx-auto max-w-[900px]">
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_0.7fr]">
            <ClayCard size="lg">
              ...
              <div className="mt-8 space-y-6">
                {terms.map(([title, text], index) => (
                  <section key={title} className="border-b-[3px] border-clay-mint-border pb-5 last:border-b-0">
                    <Type variant="display-md" className="text-clay-green-700!">{index + 1}. {title}</Type>
                    <Type variant="body" className="mt-2">{text}</Type>
                  </section>
                ))}
              </div>
            </ClayCard>
            <ClayCard tone="mint" size="lg">
              ...
              <div className="mt-8 rounded-xl border-[3px] border-clay-mint-border bg-white p-6" style={{ boxShadow: "0 3px 0 0 #B9DFA0, 0 6px 12px oklch(0 0 0 / 0.06)" }}>
